...
APP_TITLE = "AI PD Studio V7"
APP_SUBTITLE = "카드뉴스 컨펌 → 수동 디렉션 재작성 → 이미지/쇼츠/롱폼 확장"
TABLE_NAME = "content_history"
HISTORY_LIMIT = 20
SCORE_KEYS = ["insight_score", "copy_score", "logic_score", "authority_score", "risk_score", "generic_score", "upload_ready_score"]
LOCAL_SCORE_KEYS = ["local_generic_score"]
...
CARD_SCHEMA = {
    ...
    "card_news": [
        {"page": 1, "role": "hook", "headline": "", "body": "", "save_point": "", "insight_device": "", "layout_type": "", "design_mood": ""}
    ],
    "quality_gate": {},
    "improvement_report": {},
}
...
근거 처리 규칙:
- 실제 레퍼런스에 없는 외부 정보는 '확정 사실'처럼 쓰지 말고 evidence_notes에 '추론 확장' 또는 '검증 필요'로 표시한다.
...
- save_point는 '저장 포인트'다. 화면에 넣을 수도 있고 게시글 캡션/카드 하단 메모로도 쓸 수 있는 압축 문장이다. 빈칸 금지.
- save_point는 본문 반복 요약이 아니라 독자가 캡처하거나 저장할 이유가 되는 한 줄이어야 한다.
...
def normalize_score_value(value: Any, key: str) -> int:
    if value is None:
        return 0
    try:
        num = float(value)
        ...
        return 0
    if key not in LOCAL_SCORE_KEYS and 0 <= num <= 10:
        num *= 10
...
def normalize_scores(gate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in scores.items():
        normalized[key] = normalize_score_value(value, key)
    gate["scores"] = normalized
    return gate

def get_card_save_point(card: Dict[str, Any]) -> str:
    return card.get("save_point") or card.get("save_line") or ""

def all_card_text(package: Dict[str, Any]) -> str:
    chunks: List[str] = []
    for card in package.get("card_news", []) or []:
        chunks.extend([str(card.get("headline", "")), str(card.get("body", "")), str(get_card_save_point(card))])
    return "\n".join(chunks)
...
def deterministic_quality_gate(package: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for card in package.get("card_news", []) or []:
        if not get_card_save_point(card).strip():
            weak_hits.append({"line": f"{card.get('page', '?')}장 save_point 빈칸", "reason": "저장 포인트가 비어 있습니다.", "fix_direction": "카드의 핵심 판단 기준을 한 줄로 압축하세요."})
...
def merge_audit_with_local(audit: Dict[str, Any], package: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if local["generic_score"] > 0:
        ...
        gate["critical_issues"].append("로컬 하드게이트: 약한 카피/빈 저장 포인트가 감지되어 업로드 가능 판정을 차단했습니다.")
    return gate
...
목표 점수: upload_ready_score {min_score} 이상. 모든 score는 0~100 기준.
재작성 회차: {attempt}
사용자 개선 방향:
...
4. 약한 문장, 정보형 제목, 빈 save_point를 모두 제거한다.
5. 카드뉴스 원고만 다시 만든다. 이미지/쇼츠/롱폼은 만들지 않는다.
"""
...
def package_to_rows(package: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    brief = package.get("auto_brief", {}) or {}
    for card in package.get("card_news", []) or []:
        rows.append({"type":"card_news", "index":card.get("page", ""), "headline":card.get("headline", ""), "body":card.get("body", ""), "save_point":get_card_save_point(card), "role":card.get("role", ""), "layout_or_visual":card.get("layout_type", ""), "prompt":"", "image_ratio":brief.get("image_ratio", "")})
    expansion = package.get("expansion", {}) or {}
    for item in (expansion.get("image_production", {}) or {}).get("cards", []) or []:
        rows.append({"type":"image_prompt", "index":item.get("page", ""), "headline":"", "body":item.get("korean_direction", ""), "save_point":"", "role":"", "layout_or_visual":item.get("layout_direction", ""), "prompt":item.get("english_prompt", ""), "image_ratio":brief.get("image_ratio", "")})
    shorts = expansion.get("shorts", {}) or {}
    for idx, scene in enumerate(shorts.get("scenes", []) or [], 1):
        rows.append({"type":"shorts_scene", "index":idx, "headline":scene.get("caption", ""), "body":scene.get("narration", ""), "save_point":shorts.get("core_claim", ""), "role":scene.get("time", ""), "layout_or_visual":scene.get("visual", ""), "prompt":stringify(scene.get("video_prompt", {})), "image_ratio":brief.get("image_ratio", "")})
    longform = expansion.get("longform", {}) or {}
    if longform:
        rows.append({"type":"longform_script", "index":1, "headline":longform.get("title", ""), "body":longform.get("script", ""), "save_point":longform.get("opening_hook", ""), "role":"longform", "layout_or_visual":"", "prompt":"", "image_ratio":brief.get("image_ratio", "")})
    return rows
...
def render_card_package(package: Dict[str, Any], min_score: int) -> None:
    ...
    with tabs[0]:
        for i, card in enumerate(package.get("card_news", []) or [], 1):
            with st.expander(f"{card.get('page', i)}장 · {card.get('role','')} · {card.get('headline','')}", expanded=i <= 2):
                st.markdown(f"**헤드라인**\n\n{card.get('headline','')}")
                st.markdown(f"**본문**\n\n{card.get('body','')}")
                st.markdown(f"**저장 포인트**\n\n{get_card_save_point(card)}")
    ...
    with tabs[6]:
        ...
        st.download_button("CSV 다운로드", csv_text, file_name="ai_pd_studio_v7_content.csv", mime="text/csv", use_container_width=True)
        st.download_button("JSON 다운로드", stringify(package), file_name="ai_pd_studio_v7_output.json", mime="application/json", use_container_width=True)
...
st.markdown(f"""<div class='hero-card'><h1>{APP_TITLE}</h1><p>{APP_SUBTITLE}</p><p class='small-muted'>점수는 0~100 기준으로 정규화하고, 자동 개선 실패 시 사용자가 직접 디렉션을 넣어 재작성할 수 있습니다.</p></div>""", unsafe_allow_html=True)
