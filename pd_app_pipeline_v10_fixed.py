import csv
import io
import os
import runpy

import streamlit as st

APP_SUBTITLE = "상시 디렉션 입력 · 추론 설계도 · 리서치 출처 · 카드뉴스 컨펌 후 확장"
HISTORY_LIMIT = 20

# If the user changes to the expansion menu after generation, Streamlit keeps
# current in session_state, but the original V10 only checks approved. Promote
# the current generated package so expansion can always proceed.
if st.session_state.get("current") and not st.session_state.get("approved"):
    st.session_state.approved = st.session_state.current

_original_button = st.button


def _button_with_manual_confirm(label, *args, **kwargs):
    if label == "이 카드뉴스 내용 컨펌":
        kwargs["disabled"] = False
        label = "현재 원고로 제작 확정"
        kwargs.setdefault("type", "primary")
        kwargs["help"] = kwargs.get("help") or "품질 게이트가 수정 필요이어도 현재 원고를 기준으로 제작 확정합니다."
    return _original_button(label, *args, **kwargs)


st.button = _button_with_manual_confirm

runpy.run_path(
    os.path.join(os.path.dirname(__file__), "pd_app_pipeline_v10.py"),
    init_globals={"APP_SUBTITLE": APP_SUBTITLE, "HISTORY_LIMIT": HISTORY_LIMIT, "csv": csv, "io": io},
)
