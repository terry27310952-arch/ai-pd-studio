...
레퍼런스를 요약하지 말고, 흥행 문법을 해부해 새 콘텐츠를 끝까지 보게 만든다.

내부 절차:
1. 입력값 의심: 레퍼런스가 팔고 싶은 감정, 욕망, 불안, 논쟁을 찾는다.
2. 팩트/앵글 뱅크: 추상어 대신 구체명사, 이해관계, 충돌, 사례, 체크포인트를 만든다. 실시간 사실확인은 못 하므로 불확실한 것은 '확인 필요'로 표시한다.
3. 후킹 전쟁실: 약한 오프닝을 버리고 가장 강한 후킹 구조를 고른다.
...
금지 표현: 살펴보겠습니다, 주목받고 있습니다, 중요한 영향을 미칠 것입니다, 균형 잡힌 시각이 필요합니다, 미래 산업에 기회가 있습니다, 최신 정보를 확인하세요, 관련 아이콘, 깔끔한 다이어그램, 미래 도시 이미지, 그래픽 아이콘.
반드시 JSON만 출력한다.
"""

SHORTS_DOCTOR_SYSTEM = """
너는 1분 숏폼 전문 작가이자 리텐션 PD다.
입력된 패키지의 one_minute_shorts만 갈아엎는다.
카드뉴스식 설명, 발표자료식 문장, 추상 화면, 약한 CTA를 제거한다.
0-3초 첫 문장은 강한 주장/반전/충격 해석이어야 한다.
7~9컷, 구어체, 짧은 호흡. visual은 실제 장면/오브젝트/인물/화면/사건으로 쓴다.
video_prompt.english_prompt는 반드시 카메라 무빙으로 시작한다.
반드시 JSON만 출력한다.
"""

CARDNEWS_DOCTOR_SYSTEM = """
너는 한국 커뮤니티 인기글 감각을 가진 카드뉴스 전문 편집자다.
입력된 패키지의 card_news만 다시 쓴다.
목표는 예쁜 정리본이 아니라 독자가 첫 장에서 멈추고 저장/공유하고 싶게 만드는 카드뉴스다.
정보 요약체, 강의체, 보고서체를 금지한다.
각 장 헤드라인은 독자의 변명, 불안, 욕망, 자기합리화 중 하나를 찌른다.
1장은 공감/충격 후킹, 2장은 문제 정의, 중반은 구체적 원인/구조, 후반은 전환/해결, 마지막 장은 행동 유도.
...
FINAL_SCHEMA = {
    ...
    "pd_brain": {"real_intent":"", "viewer_desire":"", "viewer_anxiety":"", "conflict":"", "contrarian_claim":"", "comment_trigger":"", "fact_angle_bank":{"concrete_entities":[], "stakes":[], "tensions":[], "questions":[], "checkpoints":[], "needs_verification":[]}, "hook_war_room":{"rejected_hooks":[], "selected_hook":"", "why_selected":""}, "weakness_removed":[]},
...
강제 품질 기준:
1. auto_brief에서 주제/타겟/목표/CTA/금지표현을 레퍼런스 기반으로 추론한다.
...
4. fact_angle_bank에는 구체명사를 많이 넣고, 불확실한 사실은 needs_verification에 넣는다.
5. 숏폼은 7~9컷, 정보 나열이 아니라 긴장 상승 구조여야 한다.
6. image_prompt와 video_prompt에는 {image_ratio}에 맞는 composition을 반드시 포함한다.
7. video_prompt는 영어로 쓰고 반드시 카메라 무빙으로 시작한다.
...
이미지/영상 비율: {image_ratio}
반환 JSON 형식: {{"one_minute_shorts": {{"title":"", "core_claim":"", "scenes": [{{"time":"0-3초", "narration":"", "caption":"", "visual":"", "edit_point":"", "image_prompt":{{"korean_direction":"", "english_prompt":"", "negative_prompt":"text, watermark, logo"}}, "video_prompt":{{"korean_direction":"", "english_prompt":""}}}}]}}, "quality_patch": {{"shorts_rewrite_reason": [], "shorts_hook_score": 0, "shorts_retention_score": 0, "shorts_scene_score": 0}}}}
패키지:
...
이미지 비율: {image_ratio}
카드뉴스 장수: {card_count}
반환 JSON 형식: {{"card_news": [{{"page":1, "headline":"", "body":"", "design_mood":"", "image_direction":"", "image_prompt_en":"", "key_caption":""}}], "quality_patch": {{"cardnews_rewrite_reason": [], "cardnews_hook_score": 0, "cardnews_save_score": 0, "cardnews_emotion_score": 0}}}}
패키지:
{json.dumps(package, ensure_ascii=False, indent=2)}
"""
...
def package_to_rows(package: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for idx, scene in enumerate(shorts.get("scenes", []) or [], 1):
        rows.append({"type":"shorts_scene", "index":idx, "title":shorts.get("title", ""), "core_claim":shorts.get("core_claim", ""), "time":scene.get("time", ""), "headline_or_caption":scene.get("caption", ""), "body_or_narration":scene.get("narration", ""), "visual":scene.get("visual", ""), "edit_point":scene.get("edit_point", ""), "image_prompt":scene.get("image_prompt", {}), "video_prompt":scene.get("video_prompt", {}), "image_ratio":brief.get("image_ratio", "")})
    for idx, card in enumerate(package.get("card_news", []) or [], 1):
        rows.append({"type":"card_news", "index":card.get("page", idx), "title":"", "core_claim":"", "time":"", "headline_or_caption":card.get("headline", ""), "body_or_narration":card.get("body", ""), "visual":card.get("image_direction", ""), "edit_point":card.get("design_mood", ""), "image_prompt":card.get("image_prompt_en", ""), "video_prompt":"", "image_ratio":brief.get("image_ratio", "")})
    return rows
...
def render_history_page() -> None:
    st.subheader("히스토리")
    st.caption("브라우저 새로고침에도 유지되도록 history_store.json에 저장합니다. 단, Streamlit 서버가 재배포/재시작되면 파일 저장소가 초기화될 수 있습니다.")
    ...
    selected_package = st.session_state.history[labels.index(selected)]
    brief = selected_package.get("auto_brief", {}) or {}
    st.markdown(f"### {brief.get('topic', '무제')}")
    st.json({"generated_at": selected_package.get("generated_at"), "auto_brief": brief, "reference_preview": selected_package.get("reference_preview", "")})
...
st.markdown(f"""<div class='hero-card'><h1>{APP_TITLE}</h1><p>{APP_SUBTITLE}</p><p class='small-muted'>요약봇 말고 PD 사고회로. 히스토리는 파일 저장 방식으로 새로고침에도 유지됩니다.</p></div>""", unsafe_allow_html=True)
