...
APP_TITLE = "AI PD Studio V6"
APP_SUBTITLE = "업로드 불가 → 개선안 → 자동 재작성 → 컨펌 후 제작 확장"
...
CARD_SCHEMA = {
    ...
    "insight_board": {
        ...
        "proof_logic":"", "dangerous_reading":"", "sharp_reframe":"", "must_use_lines":[], "must_not_use_lines":[], "card_flow":[]
    },
    "card_news": [
        {"page":1, "role":"hook", "headline":"", "body":"", "save_line":"", "insight_device":"", "layout_type":"", "design_mood":""}
    ],
    "quality_gate": {},
    "improvement_report": {},
}
...
지금 단계에서는 카드뉴스 원고의 뼈대가 되는 insight_board만 만든다.
...
이미지/영상/쇼츠/롱폼은 만들지 말고 카드뉴스 원고만 만든다.

강제 규칙:
- headline은 정보 제목이 아니라 주장형 문장이어야 한다.
- body는 설명문이 아니라 짧은 논리여야 한다.
- save_line은 독자가 저장하고 싶은 문장이어야 한다.
...
업로드 불가 또는 수정 필요 판정을 내릴 때는 반드시 '어떻게 고치면 업로드 가능해지는지'까지 제시한다.

반환 JSON 형식:
{
  "verdict":"업로드 가능 | 수정 필요 | 폐기 후 재작성",
  "scores":{"insight_score":0,"copy_score":0,"logic_score":0,"authority_score":0,"risk_score":0,"generic_score":0,"upload_ready_score":0},
  "weak_lines":[{"line":"", "reason":"", "fix_direction":""}],
  "critical_issues":[],
  "rewrite_actions":[],
  "setting_recommendations":{"tone_hint":"", "angle_shift":"", "card_count":"", "structure_change":"", "opening_strategy":""},
  "upgrade_brief":"업로드 가능 상태로 만들기 위한 핵심 개선안"
}
반드시 JSON만 출력한다.
"""
...
심사 결과를 변명하지 말고, 같은 소재를 더 강한 인사이트와 카피로 다시 만든다.

목표:
- upload_ready_score 85 이상을 노린다.
- 약한 문장을 모두 제거한다.
- 헤드라인은 정보 제목이 아니라 관점 문장으로 바꾼다.
- 원고 자체가 업로드 가능해야 한다. 이미지빨로 덮지 마라.
- 권위자 인용은 가능하지만, 매 장마다 해석과 판단 기준이 있어야 한다.

반환 JSON 형식:
{
  "revised_card_package": {},
  "improvement_report": {
    "why_failed":"",
    "core_fix":"",
    "rewritten_strategy":"",
    "changed_lines":[{"before":"", "after":"", "reason":""}],
    "setting_recommendations":{"tone_hint":"", "angle_shift":"", "structure_change":""}
  }
}
반드시 JSON만 출력한다.
"""
...
def merge_audit_with_local(audit: Dict[str, Any], package: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if local["generic_score"] > 0:
        ...
        gate["critical_issues"].append("로컬 하드게이트: 약한 카피 패턴이 감지되어 업로드 가능 판정을 차단했습니다.")
    return gate
...
설정: 출력 언어={output_language}, 이미지 비율={image_ratio}, 카드뉴스 장수={card_count}, 플랫폼 힌트={platform_hint}, 톤 힌트={tone_hint}, 주제 덮어쓰기={override_topic}
출력 스키마 예시:
...
def build_rewrite_prompt(card_package: Dict[str, Any], audit: Dict[str, Any], min_score: int, attempt: int) -> str:
    return f"""
...
2. 같은 소재를 업로드 가능한 카드뉴스로 다시 쓴다.
3. 약한 문장과 정보형 제목을 모두 제거한다.
4. 카드뉴스 원고만 다시 만든다. 이미지/쇼츠/롱폼은 만들지 않는다.
"""
...
def audit_and_improve(card_package: Dict[str, Any], model: str, min_score: int, max_rewrites: int) -> Dict[str, Any]:
    attempts = []
    current = card_package
    for attempt in range(0, max_rewrites + 1):
        ...
        if attempt >= max_rewrites:
            current.setdefault("improvement_report", {})
            current["improvement_report"]["attempts"] = attempts
            current["improvement_report"]["next_action"] = "자동 재작성 한도에 도달했습니다. 하드게이트 탭의 setting_recommendations를 보고 기획 설정을 바꾸거나, 레퍼런스/주제를 더 구체화하세요."
            return current
        rewrite = call_llm(REWRITE_SYSTEM, build_rewrite_prompt(current, gate, min_score, attempt + 1), model, 0.35)
...
def package_to_rows(package: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    brief = package.get("auto_brief", {}) or {}
    for card in package.get("card_news", []) or []:
        rows.append({"type":"card_news", "index":card.get("page", ""), "headline":card.get("headline", ""), "body":card.get("body", ""), "save_line":card.get("save_line", ""), "role":card.get("role", ""), "layout_or_visual":card.get("layout_type", ""), "prompt":"", "image_ratio":brief.get("image_ratio", "")})
    expansion = package.get("expansion", {}) or {}
    for item in (expansion.get("image_production", {}) or {}).get("cards", []) or []:
        rows.append({"type":"image_prompt", "index":item.get("page", ""), "headline":"", "body":item.get("korean_direction", ""), "save_line":"", "role":"", "layout_or_visual":item.get("layout_direction", ""), "prompt":item.get("english_prompt", ""), "image_ratio":brief.get("image_ratio", "")})
    shorts = expansion.get("shorts", {}) or {}
    for idx, scene in enumerate(shorts.get("scenes", []) or [], 1):
        rows.append({"type":"shorts_scene", "index":idx, "headline":scene.get("caption", ""), "body":scene.get("narration", ""), "save_line":shorts.get("core_claim", ""), "role":scene.get("time", ""), "layout_or_visual":scene.get("visual", ""), "prompt":stringify(scene.get("video_prompt", {})), "image_ratio":brief.get("image_ratio", "")})
    longform = expansion.get("longform", {}) or {}
    if longform:
        rows.append({"type":"longform_script", "index":1, "headline":longform.get("title", ""), "body":longform.get("script", ""), "save_line":longform.get("opening_hook", ""), "role":"longform", "layout_or_visual":"", "prompt":"", "image_ratio":brief.get("image_ratio", "")})
    return rows
...
def render_card_package(package: Dict[str, Any]) -> None:
    ...
    status = package.get("status", "draft")
    box_class = "ok-box" if is_upload_ready(package, 85) else "warn-box"
    st.markdown(f"<div class='{box_class}'><b>상태:</b> {status} &nbsp; <b>판정:</b> {verdict or '판정 없음'}</div>", unsafe_allow_html=True)
    if verdict:
        cols = st.columns(4)
        for idx, key in enumerate(["insight_score", "copy_score", "local_generic_score", "upload_ready_score"]):
            with cols[idx]:
                st.metric(key, scores.get(key, "-"))
    tabs = st.tabs(["카드뉴스 원고", "개선 리포트", "인사이트 보드", "하드게이트", "확장 결과", "CSV/JSON"])
    with tabs[0]:
    ...
    with tabs[5]:
        ...
        st.download_button("CSV 다운로드", csv_text, file_name="ai_pd_studio_v6_content.csv", mime="text/csv", use_container_width=True)
        st.download_button("JSON 다운로드", stringify(package), file_name="ai_pd_studio_v6_output.json", mime="application/json", use_container_width=True)
...
st.markdown(f"""<div class='hero-card'><h1>{APP_TITLE}</h1><p>{APP_SUBTITLE}</p><p class='small-muted'>불합격하면 이유만 보여주지 않고, 개선안과 재작성까지 자동으로 시도합니다.</p></div>""", unsafe_allow_html=True)
...
min_upload_score = st.sidebar.slider("업로드 가능 최소 점수", 70, 95, 85, 5)
...
if menu == "카드뉴스 기획":
    ...
    with left:
        ...
        if st.session_state.current_package:
            ...
            if not ready:
                st.warning("현재 원고는 업로드 가능 기준을 통과하지 못했습니다. 개선 리포트를 확인하거나 재생성하세요.")
    ...
    if run_button:
        if not reference_text.strip():
            ...
            if is_upload_ready(card_package, min_upload_score):
                ...
                status.warning("자동 개선 후에도 업로드 기준을 통과하지 못했습니다. 개선 리포트와 기획 설정 추천을 확인하세요.")
