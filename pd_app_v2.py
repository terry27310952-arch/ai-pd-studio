...
.score-card { padding: 1rem; border: 1px solid rgba(120,120,120,.18); border-radius: 16px; }
</style>
...
def render_history() -> None:
    st.subheader("히스토리")
    st.caption("브라우저 새로고침에는 유지됩니다. Streamlit 서버 재시작/재배포까지 완전 보존하려면 Supabase DB 연결이 필요합니다.")
...
st.markdown(f"""<div class='hero-card'><h1>{APP_TITLE}</h1><p>{APP_SUBTITLE}</p><p class='small-muted'>정리본에서 멈추지 않고, 관점 보드 → 닥터 리라이트 → 업로드 심사까지 통과시킵니다.</p></div>""", unsafe_allow_html=True)
...
if menu == "히스토리":
    ...
    if run_button:
        if not reference_text.strip():
            ...
            status.success("생성 완료. 업로드 판정까지 확인하세요.")
